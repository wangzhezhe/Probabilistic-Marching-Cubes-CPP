...
//#pragma omp parallel for
    for (int i = 0; i <= max_index; ++i)
    {
        ...
        // compute the cross prob for each sample
        
        int numCrossings = 0;
        std::vector<double> cross_prob_array;
        for (int n = 0; n < num_samples; ++n)
        {
            std::cout << R.coeff(n, 0) << " " << R.coeff(n,1) << " " << R.coeff(n,2) << " " << R.coeff(n,3) << "\n";
            /*
            ...
            */
            //double cross_prob = (1.0 * numCrossings) / (1.0 * num_samples);
            //cross_prob_array.push_back(cross_prob);
        }
